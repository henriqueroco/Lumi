...
public class ClienteBean implements Serializable {
	...
	private Predicate[] getSearchPredicates(Root<Cliente> root) {
		...
		String cpf = this.example.getCpf();
		if (cpf != null && !"".equals(cpf)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("cpf")),
					'%' + cpf.toLowerCase() + '%'));
		}
		ESexo sexo = this.example.getSexo();
		if (sexo != null) {
			predicatesList.add(builder.equal(root.get("sexo"), sexo));
		}
		String celular = this.example.getCelular();
		if (celular != null && !"".equals(celular)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("celular")),
					'%' + celular.toLowerCase() + '%'));
		}
