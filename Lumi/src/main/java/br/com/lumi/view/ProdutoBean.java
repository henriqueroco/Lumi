...
public class ProdutoBean implements Serializable {
	...
	private Predicate[] getSearchPredicates(Root<Produto> root) {
		...
		String descricaoMarca = this.example.getDescricaoMarca();
		if (descricaoMarca != null && !"".equals(descricaoMarca)) {
			predicatesList.add(builder.like(
					builder.lower(root.<String> get("descricaoMarca")),
					'%' + descricaoMarca.toLowerCase() + '%'));
		}
		ETipoEmbalagem tipoEmbalagem = this.example.getTipoEmbalagem();
		if (tipoEmbalagem != null) {
			predicatesList.add(builder.equal(root.get("tipoEmbalagem"),
					tipoEmbalagem));
		}
		EUnidadeMedida unidadeMedidaEmbalagem = this.example
				.getUnidadeMedidaEmbalagem();
		if (unidadeMedidaEmbalagem != null) {
			predicatesList
					.add(builder.equal(root.get("unidadeMedidaEmbalagem"),
							unidadeMedidaEmbalagem));
		}
		EFormaUtilizacao formaUtilizacao = this.example.getFormaUtilizacao();
		if (formaUtilizacao != null) {
			predicatesList.add(builder.equal(root.get("formaUtilizacao"),
					formaUtilizacao));
		}
